...
class ImgSaver:
    ...
    def save_img(self, img: torch.Tensor, filename: str, normalize: bool = True):
        torch_save_img(
            img.cpu(), self.root / filename, normalize=normalize, nrow=self.nrow
        )

    def save_ann(self, ann: torch.Tensor, filename: str):
        cv2.imwrite(
            str(self.root / filename),
            self.visualizer.visualize(ann.cpu()),
        )

    def save_pred(self, pred: torch.Tensor, filename: str):
